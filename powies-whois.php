...
Text Domain: powies-whois
Domain Path: /languages
*/

//Define some stuff
define( 'PWHOIS_VERSION', '0.9.34');
define( 'PWHOIS_PLUGIN_DIR', dirname( plugin_basename( __FILE__ ) ) );
load_plugin_textdomain( 'powies-whois', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
...
if( ! function_exists('pwhois_create_menu') )
{
	...
	{
		add_options_page(__('Powie\'s WHOIS Setup', 'powies-whois'), __('Powie\'s WHOIS Setup', 'powies-whois'), 'manage_options', PWHOIS_PLUGIN_DIR.'/pwhois_settings.php');
	}
	add_action('admin_menu', 'pwhois_create_menu');
}
...
function pwhois_show( $atts ) {
	...
	$sc.= '<legend>'.__('Domain', 'powies-whois').'</legend>';
	...
	$sc.='</select>';
	$sc.=' <input type="submit" id="whoissubmit" name="whoissubmit" value="'.__("Check", 'powies-whois').'" /></form>
		   </div>';
	$sc.= '<div id="pwhois_work" style="display:none;"><img src="'.includes_url().'images/spinner.gif" alt="wait" /> '.__("Waiting for domain lookup!", 'powies-whois').'</div>';
...
function pwhois_activate() {
    ...
    if ( get_option('display-on-free') == "") {
        update_option('display-on-free',__('Domain is available'));
    }
    if ( get_option('display-on-connect') == "") {
        update_option('display-on-connect',__('Domain is unavailable'));
    }
}
...
wp_localize_script( 'pwhois-ajax-request', 'pWhoisAjax',
	array(  'ajaxurl' => admin_url( 'admin-ajax.php' ), 'enter_domain' => __('Please enter Domain', 'powies-whois') )
);
*/
if( ! function_exists( 'whois_scripts' ) )
{
	function whois_scripts()
	{
		wp_enqueue_script('pwhois-ajax-request', plugin_dir_url( __FILE__ ).'pwhois.js', array( 'jquery' ) );
		// declare the URL to the file that handles the AJAX request (wp-admin/admin-ajax.php)
		wp_localize_script( 'pwhois-ajax-request', 'pWhoisAjax', array('ajaxurl' => admin_url('admin-ajax.php'), 'enter_domain' => __('Please enter Domain', 'powies-whois') ) );
	}
...
function pwhois_post(){
	...
	if (! wp_verify_nonce($_POST['post_nonce'], 'pwhoisnonce') ) die('Security check');
		...
		//check that domain is realy domainified
        $dom = filter_var($domain.$tld, FILTER_VALIDATE_DOMAIN);
		$result=$whois->lookup($dom, $PWHOIS_SERVERS);
		...
		if (get_option('show-whois-output') == 1) {
			$msg.=get_option('before-whois-output').nl2br(utf8_encode($result)).get_option('after-whois-output');
		}
...
class psWhois{
	public function lookup($domain,$whoisservers) {
		...
		elseif(preg_match("/^([-a-z0-9]{2,100})\.([a-z\.]{2,8})$/i",$domain))
		{
			...
			if(!$server) {
				return __('Error: No appropriate Whois server found for '.$domain.' domain!', 'powies-whois');
			}
			...
			return __('Invalid Input', 'powies-whois');
	}
...
function pWhoisTimeout() {
	$msg = __('Whois Server timeout', 'powies-whois');
	$response = json_encode( array( 'success' => true , 'msg' => $msg ) );
	header( "Content-Type: application/json" );
	echo $response;
	die();
